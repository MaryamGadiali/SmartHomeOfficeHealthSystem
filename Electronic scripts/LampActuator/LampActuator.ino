...
void setup()
{
  ...
  HubCommunication::sendESPNowMessage(26, "Off");
  lastSendTime = millis();
...
void loop()
{
  ...
  {                                                // Check if data is available
    ...
    else if (command=="3"){ //Turn on
      if (digitalRead(relayPin))
      {
        digitalWrite(relayPin, LOW);
      }
    }
    else if (command=="4"){ //Turn off
      if (!digitalRead(relayPin))
      {
  ...
  if (millis() - lastSendTime > sendInterval)
    {
        ...
        HubCommunication::sendESPNowMessage(26, state);
        Serial.println("Sent command 26");
        lastSendTime = millis();
    }
  ...
  if (xQueueReceive(messageQueue, &message, 0))
  {
    ...
    case 19: // Factory reset
    {
      ...
      FileHandling::deleteFile();
      digitalWrite(relayPin, HIGH); // Turns off lamp
      ESP.restart();
      break;
    }
